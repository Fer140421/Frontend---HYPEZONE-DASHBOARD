...
export class ReservaService {
  ...
  async crear(input: ReservaInput): Promise<string> {
    if (!input.cliente.id) throw new Error('Selecciona un cliente registrado.');
    if (!input.detalles.length) throw new Error('Agrega al menos un producto.');
    const ids = input.detalles.map(({ producto }) => producto.id);
    if (ids.some((id) => !id) || new Set(ids).size !== ids.length) throw new Error('La selección de productos no es válida.');
    const details = input.detalles.map(({ producto, precioAcordado }): ReservaDetalle => ({
      productoId: producto.id!, nombreProducto: producto.nombre, precioAcordado: Number(precioAcordado),
    }));
    const total = details.reduce((sum, item) => sum + item.precioAcordado, 0);
    const anticipo = Number(input.anticipo ?? 0);
    if (details.some((item) => !Number.isFinite(item.precioAcordado) || item.precioAcordado < 0)) throw new Error('El precio acordado no es válido.');
    if (!Number.isFinite(anticipo) || anticipo < 0 || anticipo > total) throw new Error('El anticipo debe estar entre 0 y el total.');
    ...
    await runTransaction(this.firestore, async (tx) => {
      ...
      products.forEach((snapshot) => {
        ...
        if (producto.activo === false || producto.estado !== 'disponible') throw new Error(`El producto "${producto.nombre}" ya no está disponible.`);
      });
      const timestamp = serverTimestamp();
      const anticipos: ReservaPago[] = anticipo ? [{ monto: anticipo, metodoPago: input.metodoAnticipo ?? 'efectivo', fecha: input.fechaReserva }] : [];
      tx.set(reservaRef, removeUndefinedDeep<Reserva>({
        schemaVersion: 1, clienteId: input.cliente.id!, clienteNombre: input.cliente.nombreCompleto,
        clienteTelefono: input.cliente.celular, clienteCi: input.cliente.ci, detalles: details, total, anticipos,
        saldoPendiente: total - anticipo, estado: 'activa', fechaReserva: input.fechaReserva,
        fechaVencimiento: input.fechaVencimiento || undefined, notas: input.notas || undefined, activo: true,
  ...
  async registrarAbono(reservaId: string, pago: ReservaPago): Promise<void> {
    ...
    await runTransaction(this.firestore, async (tx) => {
      ...
      if (!Number.isFinite(monto) || monto <= 0 || monto > Number(reserva.saldoPendiente)) throw new Error('El abono debe ser mayor a 0 y no superar el saldo pendiente.');
      const anticipos = [...(reserva.anticipos ?? []), { ...pago, monto }];
      tx.update(ref, { anticipos, saldoPendiente: Number(reserva.total) - totalAnticipos({ anticipos }), updatedAt: serverTimestamp() });
    });
  }
  ...
  async cancelar(reservaId: string): Promise<void> {
    ...
    await runTransaction(this.firestore, async (tx) => {
      ...
      const refs = reserva.detalles.map((item) => doc(this.firestore, `productos/${item.productoId}`));
  ...
  async convertirEnVenta(reservaId: string, metodoPago: MetodoPago, fechaVenta: string): Promise<string[]> {
    ...
    await runTransaction(this.firestore, async (tx) => {
      ...
      if (!snapshot.exists()) throw new Error('Reserva no encontrada.');
      const reserva = snapshot.data() as Reserva;
      if (reserva.estado !== 'activa') throw new Error('Solo se pueden convertir reservas activas.');
      const productRefs = reserva.detalles.map((item) => doc(this.firestore, `productos/${item.productoId}`));
      const products = await Promise.all(productRefs.map((ref) => tx.get(ref)));
      ...
      tx.set(operacionRef, removeUndefinedDeep({
        reservaId, total: reserva.total, cantidadDetalles: reserva.detalles.length, clienteId: reserva.clienteId,
        clienteNombre: reserva.clienteNombre, clienteTelefono: reserva.clienteTelefono, clienteCi: reserva.clienteCi,
        metodoPago, pagos, anticipoAplicado: totalAnticipos(reserva), fechaVenta, notas: reserva.notas || undefined,
        activo: true, schemaVersion: 2, createdAt: timestamp, updatedAt: timestamp,
      }));
      reserva.detalles.forEach((detail, index) => {
        ...
        tx.set(ventaRef, removeUndefinedDeep<Partial<Venta>>({
          operacionId: operacionRef.id, reservaId, cantidadDetalles: reserva.detalles.length, totalOperacion: reserva.total,
          productoId: detail.productoId, loteId: product.loteId, nombreProducto: detail.nombreProducto, precioCompra,
